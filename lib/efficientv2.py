...
class Pip_effnetv2_aux(nn.Module):
    def __init__(self, effnet, num_nb, num_lms=68, input_size=256, net_stride=32, ts=False):
        super(Pip_effnetv2_aux, self).__init__()
        self.num_nb = num_nb
...
class Auxiliary_Classifier(nn.Module):
    _models = {'b0': {'num_repeat': [1, 2, 2, 3, 5, 8],
               ...
               'b3': {'num_repeat': [1, 2, 2, 3, 5, 8],
                      ...
                      'weight_url': 'https://api.onedrive.com/v1.0/shares/u!aHR0cHM6Ly8xZHJ2Lm1zL3UvcyFBdGlRcHc5VGNjZmlnUVpkamdZUzhhaDdtTTZLP2U9anA4VWN2/root/content',
                      'model_name': 'efficientnet_v2_b3_21k_ft1k-3da5874c.pth'},
               's': {'num_repeat': [2, 4, 4, 6, 6, 9, 6],
                     'kernel_size': [3, 3, 3, 3, 3, 3, 3],
                     'stride': [1, 2, 2, 2, 1, 2, 1],
                     'expand_ratio': [1, 4, 4, 4, 6, 6, 6],
                     'in_channel': [24, 24, 48, 64, 128, 160, 256],
                     'out_channel': [24, 48, 64, 128, 160, 256, 256],
                     'se_ratio': [None, None, None, 0.25, 0.25, 0.25, 0.25],
                     'conv_type': [1, 1, 1, 0, 0, 0, 0],
                     'is_feature_stage': [False, True, True, False, True, True, False],
                     'width_coefficient': 1.0,
                     'depth_coefficient': 1.0,
                     'train_size': 300,
                     'eval_size': 384,
    ...
    def __init__(self,
        ...
        self.tf_style_conv = tf_style_conv
        ...
        if tf_style_conv and in_spatial_shape is None:
            self.in_spatial_shape = self.cfg['eval_size']
        else:
        ...
        self.drop_connect_rates = self.get_dropconnect_rates(drop_connect_rate)
        ...
        idx = 0
        if model_name == 's':
            self.block_extractor1 = self._make_layer(stages[2:6], 0)
            self.block_extractor2 = self._make_layer(stages[3:6], 10)
            self.block_extractor3 = self._make_layer(stages[5:6], 22)
            self.block_extractor4 = self._make_layer(stages[6:], 31)
        else:
            raise NotImplementedError(f"We only implemented model name:s, but we got {model_name}!")

        head_conv_out_channels = round_filters(1280, self.cfg['width_coefficient'])
        in_channels = self.cfg["out_channel"][-1]
        
        self.head_conv1 = nn.Conv2d(in_channels=in_channels,
                                   out_channels=head_conv_out_channels,
                                   kernel_size=1,
                                   bias=bias)
        self.head_bn1 = nn.BatchNorm2d(num_features=head_conv_out_channels,
                                      eps=bn_epsilon,
                                      momentum=bn_momentum)
        self.head_conv2 = nn.Conv2d(in_channels=in_channels,
                                   out_channels=head_conv_out_channels,
                                   kernel_size=1,
                                   bias=bias)
        self.head_bn2 = nn.BatchNorm2d(num_features=head_conv_out_channels,
                                      eps=bn_epsilon,
                                      momentum=bn_momentum)
        self.head_conv3 = nn.Conv2d(in_channels=in_channels,
                                   out_channels=head_conv_out_channels,
                                   kernel_size=1,
                                   bias=bias)
        self.head_bn3 = nn.BatchNorm2d(num_features=head_conv_out_channels,
                                      eps=bn_epsilon,
                                      momentum=bn_momentum)
        self.head_conv4 = nn.Conv2d(in_channels=in_channels,
                                   out_channels=head_conv_out_channels,
                                   kernel_size=1,
                                   bias=bias)
        self.head_bn4 = nn.BatchNorm2d(num_features=head_conv_out_channels,
                                      eps=bn_epsilon,
                                      momentum=bn_momentum)
        
        self.head_act = get_activation(activation, **activation_kwargs)
        
        self.cls_layer1 = nn.Conv2d(head_conv_out_channels, num_lms, kernel_size=1, stride=1, padding=0)
        self.x_layer1 = nn.Conv2d(head_conv_out_channels, num_lms, kernel_size=1, stride=1, padding=0)
        self.y_layer1 = nn.Conv2d(head_conv_out_channels, num_lms, kernel_size=1, stride=1, padding=0)
        self.nb_x_layer1 = nn.Conv2d(head_conv_out_channels, num_nb*num_lms, kernel_size=1, stride=1, padding=0)
        self.nb_y_layer1 = nn.Conv2d(head_conv_out_channels, num_nb*num_lms, kernel_size=1, stride=1, padding=0)
        self.cls_layer2 = nn.Conv2d(head_conv_out_channels, num_lms, kernel_size=1, stride=1, padding=0)
        self.x_layer2 = nn.Conv2d(head_conv_out_channels, num_lms, kernel_size=1, stride=1, padding=0)
        self.y_layer2 = nn.Conv2d(head_conv_out_channels, num_lms, kernel_size=1, stride=1, padding=0)
        self.nb_x_layer2 = nn.Conv2d(head_conv_out_channels, num_nb*num_lms, kernel_size=1, stride=1, padding=0)
        self.nb_y_layer2 = nn.Conv2d(head_conv_out_channels, num_nb*num_lms, kernel_size=1, stride=1, padding=0)
        self.cls_layer3 = nn.Conv2d(head_conv_out_channels, num_lms, kernel_size=1, stride=1, padding=0)
        self.x_layer3 = nn.Conv2d(head_conv_out_channels, num_lms, kernel_size=1, stride=1, padding=0)
        self.y_layer3 = nn.Conv2d(head_conv_out_channels, num_lms, kernel_size=1, stride=1, padding=0)
        self.nb_x_layer3 = nn.Conv2d(head_conv_out_channels, num_nb*num_lms, kernel_size=1, stride=1, padding=0)
        self.nb_y_layer3 = nn.Conv2d(head_conv_out_channels, num_nb*num_lms, kernel_size=1, stride=1, padding=0)
        self.cls_layer4 = nn.Conv2d(head_conv_out_channels, num_lms, kernel_size=1, stride=1, padding=0)
        self.x_layer4 = nn.Conv2d(head_conv_out_channels, num_lms, kernel_size=1, stride=1, padding=0)
        self.y_layer4 = nn.Conv2d(head_conv_out_channels, num_lms, kernel_size=1, stride=1, padding=0)
        self.nb_x_layer4 = nn.Conv2d(head_conv_out_channels, num_nb*num_lms, kernel_size=1, stride=1, padding=0)
        self.nb_y_layer4 = nn.Conv2d(head_conv_out_channels, num_nb*num_lms, kernel_size=1, stride=1, padding=0)
    ...
    def _make_layer(self, stages, idx):
        blocks = []
        for stage_args in stages:
            (num_repeat, kernel_size, stride, expand_ratio,
             in_channels, out_channels, se_ratio, conv_type, is_feature_stage) = stage_args
            ...
            for _ in range(num_repeat):
                ...
                _b = conv_block(in_channels=in_channels,
                                ...
                                act_kwargs=self.activation_kwargs,
                                bn_epsilon=self.bn_epsilon,
                                bn_momentum=self.bn_momentum,
                                se_size=se_size,
                                drop_connect_rate=self.drop_connect_rates[idx],
                                bias=self.bias,
                                tf_style_conv=self.tf_style_conv,
                                in_spatial_shape=self.in_spatial_shape
                                )
                blocks.append(_b)
                idx += 1
                if self.tf_style_conv:
                    self.in_spatial_shape = _b.out_spatial_shape
                in_channels = out_channels
                stride = 1
    ...
    def forward(self, x):
        ...
        for i in range(len(x)):
            ...
            out = getattr(self, 'head_conv'+str(idx))(out)
            out = getattr(self, 'head_bn'+str(idx))(out)
            out = self.head_act(out)
            cls_map = getattr(self, 'cls_layer'+str(idx))(out)
            cls_maps.append(cls_map)
            x_map = getattr(self, 'x_layer'+str(idx))(out)
            x_maps.append(x_map)
            y_map = getattr(self, 'y_layer'+str(idx))(out)
            y_maps.append(y_map)
            nb_x_map = getattr(self, 'nb_x_layer'+str(idx))(out)
            nb_x_maps.append(nb_x_map)
            nb_y_map = getattr(self, 'nb_y_layer'+str(idx))(out)
            nb_y_maps.append(nb_y_map)
            
        return cls_maps, x_maps, y_maps, nb_x_maps, nb_y_maps
    

class EfficientNetv2_Auxiliary(nn.Module):
    def __init__(self, backbone, model_name, num_nb, num_lms=68, n_classes=1000):
        super(EfficientNetv2_Auxiliary, self).__init__()
        self.backbone = backbone
        self.auxiliary_classifier = Auxiliary_Classifier(model_name, num_nb, num_lms=num_lms, n_classes=n_classes)
        
    def forward(self, x, grad=False):
        if grad is False:
            feats, x1, x2, x3, x4, x5 = self.backbone(x)
        ...
        # for feat in feats:
        #     print(feat.shape)
        cls_maps, x_maps, y_maps, nb_x_maps, nb_y_maps = self.auxiliary_classifier(feats)
        cls_maps.append(x1)
        x_maps.append(x2)
        y_maps.append(x3)
        nb_x_maps.append(x4)
